...
class PhysicsEntity:
    def __init__(self, game, e_type, pos, size):
        self.game = game
        self.type = e_type
        self.pos = list(pos)   # We use list(pos) bcz to make position mutable for eg --> if our Player pos is  Player((100,200))it is immutable it becomes [100,200] now its list and mutable then self.pos[0] += 5
        self.size = size
